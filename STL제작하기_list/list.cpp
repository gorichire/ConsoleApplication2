...
int main() {
    ...
    for (auto it = L.begin(); it != L.end(); ++it) std::cout << *it << ' '; // 5 10 20
    std::cout << "\nfront=" << L.front() << ", back=" << L.back() << "\n";

    auto it = L.begin();  // 5
    ++it;                 // 10
    L.insert(it, 7);      // 10 앞에 7 삽입 → 5 7 10 20

    it = L.begin(); ++it; // 7
    L.erase(it);          // 7 삭제 → 5 10 20

    for (auto x_it = L.begin(); x_it != L.end(); ++x_it) std::cout << *x_it << ' ';
    std::cout << "\n";
}